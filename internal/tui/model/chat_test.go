...
func TestPhaseEventUpdatesPhase(t *testing.T) {
	...
	m.Update(event.AgentStart{QueryID: "q-1", AgentName: "Query Agent"})

	// Send a PhaseEvent
	updated, _ := m.Update(event.Phase{QueryID: "q-1", Phase: "thinking"})
...
func TestStreamDoneStopsSpinner(t *testing.T) {
	...
	updated, _ := m.Update(event.StreamDone{QueryID: "q-1"})
...
func TestToolFailMarksToolDoneWithoutErrorText(t *testing.T) {
	m := model.NewChatModel(80, 40)
	m, _ = m.Update(event.AgentStart{QueryID: "q-1", AgentName: "Deploy Agent"})
	m, _ = m.Update(event.ToolCall{QueryID: "q-1", ToolName: "helm preflight", Step: 5})
	m, _ = m.Update(event.ToolFail{
...
func TestStreamErrStopsSpinner(t *testing.T) {
	...
	updated, _ := m.Update(event.StreamErr{QueryID: "q-1", Err: fmt.Errorf("boom")})
...
func TestLLMTextDeltaWritesToLatestPhase(t *testing.T) {
	m := model.NewChatModel(80, 40)
	m.Update(event.AgentStart{QueryID: "q-1", AgentName: "Query Agent"})
	m.Update(event.Phase{QueryID: "q-1", Phase: "thinking"})

	// Phase emits LLMTextDelta
	m.Update(event.LLMTextDelta{QueryID: "q-1", Delta: "checking pod status..."})
	m.Update(event.LLMTextDelta{QueryID: "q-1", Delta: " found 3 pods"})
...
func TestToolCallNestedUnderPhase(t *testing.T) {
	...
	m.Update(event.Phase{QueryID: "q-1", Phase: "thinking"})
	m.Update(event.ToolCall{QueryID: "q-1", ToolName: "list_resources", Step: 1})

	// Phase 2: analyzing, with different tool call
	m.Update(event.Phase{QueryID: "q-1", Phase: "analyzing"})
	m.Update(event.ToolCall{QueryID: "q-1", ToolName: "get_resource", Step: 1})

	// Mark analyzing phase's tool done
	m.Update(event.ToolDone{QueryID: "q-1", ToolName: "get_resource", Step: 1, Elapsed: time.Second})
...
func TestTogglePhaseReasoning(t *testing.T) {
	...
	m.Update(event.AgentStart{QueryID: "q-1", AgentName: "Query Agent"})
	m.Update(event.Phase{QueryID: "q-1", Phase: "thinking"})
	m.Update(event.LLMTextDelta{QueryID: "q-1", Delta: "some reasoning text"})
