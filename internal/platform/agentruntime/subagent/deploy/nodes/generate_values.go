...
func GenerateValues(st *state.State) error {
	st.Emit(event.Phase{QueryID: st.QueryID, Phase: "生成配置建议"})
...
func emitValuesNotes(st *state.State, r *values.Result) {
	...
	st.Emit(event.LLMTextDelta{QueryID: st.QueryID, Delta: r.Explanation})
	if r.RiskLevel == "high" {
		st.Emit(event.LLMTextDelta{QueryID: st.QueryID, Delta: "⚠️ 配置风险等级: high，请仔细确认"})
	}
}
